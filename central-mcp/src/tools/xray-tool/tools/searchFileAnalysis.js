...
export async function handleSearchFileAnalysis(args) {
    try {
...
async function findLatestAnalysisFile() {
    try {
        ...
        const analysisFiles = files
            .filter(file => file.startsWith('xray-analysis-') && file.endsWith('.json') ||
        ...
        const fileStats = await Promise.all(analysisFiles.map(async (file) => {
            try {
...
async function searchFiles(files, args) {
...
function analyzeFileMatch(file, query, searchIn) {
    let score = 0;
    const reasons = [];
    let context_snippet;
    // Search in file name
...
function generateSearchResults(matches, args) {
...
//# sourceMappingURL=searchFileAnalysis.js.map