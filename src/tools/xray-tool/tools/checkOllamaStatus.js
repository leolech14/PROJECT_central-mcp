...
export async function handleCheckOllamaStatus(args) {
...
function generateStatusReport(ollamaUrl, status) {
...
function generateErrorReport(ollamaUrl, status) {
...
function getRecommendedModels(models) {
    ...
    return recommended.filter(modelName => models.some(model => model.name.includes(modelName.split(':')[0])));
}
