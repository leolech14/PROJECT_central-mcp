...
export async function handleGetFileSummary(args) {
    try {
        ...
        if (args.filter_by_language) {
            filteredFiles = filteredFiles.filter(file => file.language.toLowerCase() === args.filter_by_language.toLowerCase());
        }
        // Sort results
...
function generateSummary(files, metadata, args) {
